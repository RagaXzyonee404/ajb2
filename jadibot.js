  ...
  try {
    async function start() {
      ...
      const das = await makeWaSocket({
        auth: state,
        browser: [`Jadibot Md By Iox`, 'Chrome', '1.0.0'],
        ...
        if (connection == 'open') {
          ...
          await m.reply(
            `*Succesd Jadibot*\njika ada yang error segera chat .owner!!!\n\n*User :*\n _*× id : ${das.decodeJid(
              das.user.id
            )}*_\n\nJika bot mati ketik kembali *.jadibot* di group JADIBOT FREE link : https://chat.whatsapp.com/J5v1zHSYt812aKMtvc34ly\n\nNote : Jadibot Ini Bisa Mati Kapan Saja Jika Ingin On 24jam Dalam 1Bulan Bisa Order JasaRun Bot Ke Owner Murah Kok Ga Sampai Jual Tanah, Dan Owner Jadi Nomorlu`
          );
